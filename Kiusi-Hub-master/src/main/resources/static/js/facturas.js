document.addEventListener("DOMContentLoaded", function() {
    ...
    function abrirModalNotaCredito(id) {
        ...
        // Obtener la factura para sacar el pedidoId
        fetch(`/api/facturas/${id}`)
            .then(res => {
                console.log('Factura response:', res);
                if (!res.ok) {
            ...
            .then(factura => {
                ...
                console.log('Factura pedidoId:', factura.pedidoId);
                if (!factura.pedidoId) {
                    throw new Error('Esta factura no tiene pedido asociado');
                }
                // Obtener los items del pedido
                return fetch(`/api/items/pedido/${factura.pedidoId}`);
            })
            .then(res => {
    ...
    function renderizarItemsNotaCredito() {
        ...
        itemsFacturaNotaCredito.forEach((item, index) => {
            ...
            tr.innerHTML = `
                <td>
                    <input type="checkbox" id="item-${index}" onchange="calcularTotalNotaCredito()">
                </td>
                <td>
                    <input type="text" value="${item.nombreProducto}" readonly style="background: #f8f9fa; border: none; width: 100%;">
                </td>
                <td>
                    <input type="number" value="${item.cantidad}" readonly id="cantidad-facturada-${index}" style="width: 100%;">
                </td>
                <td>
                    <input type="number" value="0" min="0" max="${item.cantidad}" id="cantidad-devolver-${index}" onchange="calcularTotalNotaCredito()" style="width: 100%;">
                </td>
                <td>
    ...
    function calcularTotalNotaCredito() {
        ...
        itemsFacturaNotaCredito.forEach((item, index) => {
            ...
            const cantidadDevolver = parseInt(document.getElementById(`cantidad-devolver-${index}`).value) || 0;
        ...
        document.getElementById("totalNotaCredito").textContent = `$${total.toFixed(2)}`;
    }
    ...
    function generarNotaCredito() {
        ...
        if (itemsSeleccionados.length === 0) {
            Swal.fire({
                icon: 'warning',
                title: 'Sin productos',
                text: 'Por favor selecciona al menos un producto para devolver'
            });
            return;
        }
        ...
        const notaCredito = {
            ...
            monto: 0, // Se calcula en el backend
            motivo: motivo,
        ...
        Swal.fire({
            title: '¿Estás seguro?',
            text: 'Se generará una nota de crédito y se devolverá el stock de los productos',
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
                ...
                .then(res => {
                    if (res.ok) {
                        ...
                        return res.text().then(text => { 
                            throw new Error(text || 'Error al generar la nota de crédito'); 
                        });
                    }
                })
                .then(() => {
                    Swal.fire({
                        ...
                        text: 'Nota de crédito generada correctamente y stock devuelto',
                        timer: 1500
                    }).then(() => {
